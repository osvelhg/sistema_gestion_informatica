...
const submit = () =>
    editing.value
        ? form.put(`/estados-establecimiento/${editing.value.id}`, { onSuccess: () => (showModal.value = false) })
        : form.post('/estados-establecimiento', { onSuccess: () => (showModal.value = false) })

const destroy = async (row) => {
    if (!await confirmDanger({ title: 'Eliminar estado', text: `Se eliminará "${row.name}".`, confirmText: 'Sí, eliminar' })) return
    router.delete(`/estados-establecimiento/${row.id}`)
}
</script>
...
<template>
    <AppLayout>
        <div class="mx-auto max-w-3xl space-y-6">
            <PageHeader eyebrow="Nomencladores · Conectividad" title="Estados del Establecimiento" description="Condición operativa del punto de venta: Abierto, Cerrado, etc.">
                <template #actions>
                    <button class="app-button-primary" @click="openCreate">Nuevo estado</button>
                </template>
            </PageHeader>
                ...
                <div class="surface-card relative z-10 w-full max-w-md p-6">
                    ...
                    <form class="space-y-4" @submit.prevent="submit">
                        <div class="space-y-1.5">
                            ...
                            <input v-model="form.name" type="text" class="app-input" placeholder="Ej: Abierto" />
