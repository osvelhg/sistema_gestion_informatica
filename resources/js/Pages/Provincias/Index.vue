...
const props = defineProps({ provincias: Object, filters: Object })
const search = ref(props.filters?.search || '')
const showModal = ref(false)
const editing = ref(null)

const form = useForm({ name: '', code: '', active: true })
const columns = [
    { key: 'code', label: 'Codigo' },
    { key: 'name', label: 'Provincia' },
    { key: 'municipios_count', label: 'Municipios' },
...
watch(search, (value) => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/provincias', { search: value || undefined }, { preserveState: true, replace: true })
    }, 300)
})
...
const openEdit = (provincia) => {
    editing.value = provincia
    form.name = provincia.name
    form.code = provincia.code
    form.active = provincia.active
    form.clearErrors()
    showModal.value = true
}
...
const submit = () => {
    ...
    if (editing.value) return form.put(`/provincias/${editing.value.id}`, options)
    form.post('/provincias', options)
}

const destroy = async (provincia) => {
    if (!await confirmDanger({ title: 'Eliminar provincia', text: `Se eliminara la provincia "${provincia.name}".`, confirmText: 'Si, eliminar' })) return
    router.delete(`/provincias/${provincia.id}`)
}
</script>
...
<template>
    <AppLayout>
        <div class="mx-auto max-w-6xl space-y-6">
            <PageHeader eyebrow="Territorio" title="Provincias" description="Gestiona provincias con tablas, formularios y modales totalmente adaptados a claro y oscuro.">
                <template #actions>
                    <button type="button" class="app-button-primary" @click="openCreate">Nueva provincia</button>
                </template>
            </PageHeader>
            ...
            <BaseCard>
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Localiza provincias por nombre o codigo sin contraste pobre.</p>
                    </div>
                    <div class="w-full max-w-md">
                        <input v-model="search" type="text" placeholder="Buscar provincia..." class="app-input" />
                    </div>
                </div>
            </BaseCard>

            <DataTable client-table :columns="columns" :data="provincias">
                ...
                <template #cell-name="{ row }">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400">Division territorial</p>
                    </div>
                </template>
                <template #cell-municipios_count="{ value }">
                ...
                <template #empty>No se encontraron provincias.</template>
            </DataTable>
        </div>
        ...
        <Teleport to="body">
            <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center px-4">
                ...
                <div class="surface-card relative z-10 w-full max-w-lg p-6">
                    <div class="mb-6">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-brand-600 dark:text-brand-300">Provincia</p>
                        <h3 class="mt-2 text-xl font-semibold text-slate-950 dark:text-slate-100">{{ editing ? 'Editar provincia' : 'Nueva provincia' }}</h3>
                    </div>
