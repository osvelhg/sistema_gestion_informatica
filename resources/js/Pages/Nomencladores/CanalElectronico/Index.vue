...
function submitCreate() {
    createForm.post(route('canales-electronicos.store'), {
...
function startEdit(c) {
    editId.value        = c.id
    editForm.nombre      = c.nombre
    editForm.datacell_id = c.datacell_id ?? ''
    editForm.estado      = c.estado
}
function cancelEdit() { editId.value = null }

function submitEdit(id) {
    editForm.put(route('canales-electronicos.update', id), {
...
function confirmDelete(id) {
    if (!confirm('¿Eliminar este Canal Electrónico?')) return
    router.delete(route('canales-electronicos.destroy', id), { preserveScroll: true })
}
...
<template>
    <AppLayout title="Canales Electrónicos">
        <PageHeader title="Canales Electrónicos" subtitle="Nomenclador para tipos de canal de cobro digital">
            <template #actions>
            ...
            <div v-if="showCreate" class="p-4 border-b border-gray-200 dark:border-gray-700 bg-green-50 dark:bg-green-900/10">
                <form @submit.prevent="submitCreate" class="flex flex-wrap gap-3 items-end">
                    ...
                    <div class="w-36">
                        ...
                        <input v-model="createForm.datacell_id" type="number" :class="inputClass" placeholder="Ej: 3"/>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <tr v-if="!canales.length">
                            <td colspan="4" class="px-4 py-10 text-center text-gray-400">Sin canales registrados.</td>
                        </tr>

                        <template v-for="c in canales" :key="c.id">
                            <!-- Fila de edición inline -->
                            <tr v-if="editId === c.id" class="bg-blue-50 dark:bg-blue-900/10">
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2 text-right whitespace-nowrap space-x-2">
                                    <button @click="submitEdit(c.id)" :disabled="editForm.processing"
                            ...
                            <tr v-else class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors"
                                ...
                                <td class="px-4 py-3">
                                    <span v-if="c.estado && !c.deleted_at" class="inline-flex items-center gap-1 text-xs text-green-700 dark:text-green-400">
                                ...
                                <td class="px-4 py-3 text-right whitespace-nowrap space-x-1">
                                    <button @click="startEdit(c)" v-if="!c.deleted_at"
                                    ...
                                    <button @click="confirmDelete(c.id)" v-if="!c.deleted_at"
