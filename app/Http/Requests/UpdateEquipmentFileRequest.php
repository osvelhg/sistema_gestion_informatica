...
class UpdateEquipmentFileRequest extends FormRequest
{
    use AssertsAllowedEntityAccess;

    public function authorize(): bool
    {
        return $this->user()->can('expedientes.update');
    }
    ...
    {
        $fileId = $this->route('expediente')->id;

        return [
            'entity_id'        => 'required|exists:entidades,id',
            'department_id'    => 'required|exists:departamentos,id',
            'type'             => 'required|in:PC,Laptop',
            'inventory_number' => "required|string|max:50|unique:expedientes_equipos,inventory_number,{$fileId}",
